##DACE_PATH =/afs/pdc.kth.se/home/m/mansande/.local/lib/python3.9/site-packages/dace

lib2=add3s2
SRC_PATH=${PWD}/src/fortran
AX_PATH=${PWD}/src/python/.dacecache
ADD3S2_PATH=${PWD}/src/python/.dacecache/${lib2}

FCLAGS  = `pkg-config --cflags neko`
LIBS    = `pkg-config --libs neko`
FC      = `pkg-config --variable=compiler neko`

DEST    = nekobench
SRC	= ${SRC_PATH}/dace_math.F90 ${SRC_PATH}/dace_ax_helm_device.F90 driver.F90
OBJ	= ${SRC:.F90=.o}

foo := ax1 ax ax1 ax2 ax3 ax4 ax5 ax6 ax7 ax8 add3s2
bar := $(foreach wrd,$(foo),-I${AX_PATH}/$(wrd)/include -L${AX_PATH}/$(wrd)/build -l$(wrd)) #$(foreach wrd,$(foo),-l$(wrd))

mv_obj: $(OBJ)
	mv *dace*.o ${SRC_PATH}
	
my_test:  	
	@echo $(bar)

install:

clean:
	-rm -f *.o core *.core *.mod $(OBJ) $(DEST)

$(DEST): mv_obj 
	$(FC) $(FCLAGS) ${OBJ} -o $@ $(LIBS) $(bar)  #-I${ADD3S2_PATH}/include -L${ADD3S2_PATH}/build -l${lib2} 
	
#-I${AX_PATH}/include -L${AX_PATH}/build 

%.o: %.F90
	${FC} ${FCLAGS} -c $<


